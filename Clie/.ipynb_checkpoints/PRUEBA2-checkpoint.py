...
#Ventana.iconbitmap('Clie\grefo\cp-logo.ico')
...
g = grafo()
l = []
#img = Image.open(os.path.join(r'Clie\bola.png'))
...
def agregarA(frm, to, distancia, x1, y1, x2, y2):
    atemp = arista(frm, to, distancia, x1, y1, x2, y2,-1)
                    ...
                    if str(id_Nodo_Destino) == str(p.nombre):
                       ...
                       for q in g.listav:
                           if str(id_Nodo_Origen)== str(q.nombre):
                            relacion(str(id_Nodo_Origen), str(id_Nodo_Destino),int(dist), Toplevel(Ventana))  
...
## SubMenu Arco del SubMenu Editar
...
disjtra_Todas_Rutas.add_command(label='nomnre1 ', command=algoritmo1)
disjtra_Todas_Rutas.add_command(label='nombre2', command=algoritmo2)
Sub_Algoritmo.add_cascade(label='nombre ',menu=disjtra_Todas_Rutas)

Sub_Algoritmo.add_command(label=' Queyranne',command=algoritmo1)
Sub_Algoritmo.add_command(label=' Segundo algoritmo')
Sub_Algoritmo.add_command(label=' Tercer algoritmo')
